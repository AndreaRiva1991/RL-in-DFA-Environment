...
F_B_02_ENVIRONMENT<-function(r, g, i, T_SETTING_RUN, T_RL_REWARD, T_RL_CONTROL){
  ...
  #Reference
  RUN_SEED<-T_SETTING_RUN[RUN_INDEX == r, RUN_SEED]                  #Random number generator seed  
  RUN_SEVERITY<-T_SETTING_RUN[RUN_INDEX == r, RUN_SEVERITY]          #Type of severity distribution (LN = LogNormal / GM = Gamma)
  ...
  out<-T_RL_CONTROL[
       ...
       CLAIM_SEVERITY     := RUN_SEVERITY][,                                                                                                   
       CLAIMS_AVG_COST_LN := ifelse(START_SURVIVE == 1, rlnorm(n = .N, meanlog = ENV_SEV_LN_P1, sdlog = ENV_SEV_LN_P2), 0)][,
       CLAIMS_AVG_COST_GM := ifelse(START_SURVIVE == 1, rgamma(n = .N, shape = ENV_SEV_GM_P1, rate = ENV_SEV_GM_P2), 0)][,                                                                                                                   
       CLAIMS_AVG_COST    := ifelse(CLAIM_SEVERITY == "LN", CLAIMS_AVG_COST_LN, CLAIMS_AVG_COST_GM)][,
